...
from ..utils import gen_subclass, internal_cls
...
class LogConfig(Dict[Type["MiraiEvent"], Optional[str]]):
    ...
    def event_hook(self, app: "KritorApp") -> Callable[["MiraiEvent"], Awaitable[None]]:
        return functools.partial(self.log, app)

    async def log(self, app: "KritorApp", event: "MiraiEvent") -> None:
...
@internal_cls()
class DownloadInfo(KritorBaseModel):
    """描述一个文件的下载信息."""
...
@internal_cls()
class Announcement(KritorBaseModel):
    """群公告"""
...
@internal_cls()
class FileInfo(KritorBaseModel):
    """群组文件详细信息"""
